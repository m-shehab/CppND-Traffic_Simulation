...
void TrafficLight::cycleThroughPhases()
{
    ...
    while(true){
        ...
        if(timeSinceLastUpdate >= cycleDuration * 1000){
            ...
            auto isSent = std::async(std::launch::async,&MessageQueue<TrafficLightPhase>::send,&_msgQueue,std::move(_currentPhase));
            isSent.wait(); 
            lastUpdate = std::chrono::system_clock::now();
        }
    }
